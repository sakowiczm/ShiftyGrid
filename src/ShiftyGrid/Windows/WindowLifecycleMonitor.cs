...
internal class WindowLifecycleMonitor : IDisposable
{
    ...
    public void Start()
    {
        ...
        {
            Logger.Error("WindowLifecycleMonitor: Failed to install window event hook");
            return;
        }
        ...
        Logger.Info("WindowLifecycleMonitor: Started");
    }
    ...
    public void Stop()
    {
        ...
        _isRunning = false;
        _hookDelegate = null;
        Logger.Info("WindowLifecycleMonitor: Stopped");
    }
    ...
    private void CleanupOldAttempts()
    {
        lock (_lock)
        {
            ...
            if (toRemove.Count > 0)
            {
                Logger.Debug($"WindowLifecycleMonitor: Cleaned up {toRemove.Count} old attempt records");
            }
        }
    }
